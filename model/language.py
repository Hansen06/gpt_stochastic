...
from transformers.src.transformers import GPT2Model
...
class GPT2OUEncoder(nn.Module):
    ...
    def _init_model(self):
        self.model = GPT2Model.from_pretrained(constants.PATH2GPT)
        self.model = self.model.eval()
        # turn off all the gradients
        for param in self.model.parameters():
            param.requires_grad = self.finetune
        self.mlp = nn.Linear(self.model.wte.embedding_dim, self.hidden_dim) # wte
        ...
        self.mlp.apply(model_utils.weights_init)
        self.feature_extractor.apply(model_utils.weights_init)
        self.log_q.apply(model_utils.weights_init)
        self.C_eta.apply(model_utils.weights_init)
